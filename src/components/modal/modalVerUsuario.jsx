...
export default function ModalEditUser({ isOpen, onClose, user, onSave }) {
  ...
  React.useEffect(() => {// Si el usuario cambia, actualizamos el estado del usuario editado
    setEditedUser(user);
  }, [user]);
  ...
  return (
    <AnimatePresence>{/*AnimatePresence permite animar elementos que se están eliminando (como un modal que se cierra).*/}
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between mt-4">
                <label htmlFor="bloqueado" className="text-sm font-medium text-gray-700">
                    ¿Está bloqueado?
                </label>
                <div className='flex space-x-3 items-center'>
                    ...
                    <button
                        type="button"
                        onClick={() =>
                        setEditedUser({ ...editedUser, bloqueado: !editedUser.bloqueado })
                        }
                    ...
                    >
                        ...
                        className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-300
                            ${editedUser?.bloqueado ? 'translate-x-6' : 'translate-x-1'}`}
                        />        
                    </button>
                </div>
